...
;(function($){ "use strict";
    ...
    Iconpicker.prototype.updateArrows = function (page) {
        ...
        if (page === 1) { 
            ...
            op.table.find('.btn-previous').attr('disabled','disabled');
        }
        else {
            op.table.find('.btn-previous').removeClass('disabled');
            op.table.find('.btn-previous').removeAttr('disabled');
        }
        if (page === total_pages || total_pages === 0) { 
            op.table.find('.btn-next').addClass('disabled');
            op.table.find('.btn-next').attr('disabled','disabled');
        }
        else {
            op.table.find('.btn-next').removeClass('disabled');
            op.table.find('.btn-next').removeAttr('disabled');
        }
    };
    ...
    Iconpicker.prototype.updatePagesCount = function () {
        ...
        for (var i = 0; i < op.cols; i++) {
            ...
            if (i === 0 || i === op.cols - 1) {
                var arrow = [
                    '<button class="btn btn-primary ' + ((i === 0) ? 'btn-previous' : 'btn-next') + ' ' + op.arrowClass + '" value="' + ((i === 0) ? -1 : 1) + '">',
