...
var TinyMCE_Global = {
    ...
    content_css: urlBase + '/packages/nob/admin/cms/tinymce.css'
};
var TinyMCE = {
    menu : {
        edit: {title: 'Edit', items: 'undo redo | cut copy paste pastetext | selectall'},
        format: {title: 'Format', items: 'bold italic | formats | removeformat'},
    ...
    toolbar: "insertfile undo redo | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link | image youtube",
...
var TinyMCE_Limited_Role = {
    sadmin: {
        menu: {
            edit: {title: 'Edit', items: 'undo redo | cut copy paste pastetext | selectall'},
            insert: {title: 'Insert', items: 'link media image | youtube'},
            format: {title: 'Format', items: 'bold italic | formats | removeformat'},
            table: {title: 'Table', items: 'inserttable tableprops deletetable | cell row column'},
        ...
        toolbar: "insertfile undo redo | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link | image youtube | code",
        contextmenu: "link image | youtube | inserttable cell row column deletetable"
    },
    admin: {
        menu: {
            edit: {title: 'Edit', items: 'undo redo | cut copy paste pastetext | selectall'},
            insert: {title: 'Insert', items: 'link media image | youtube'},
            format: {title: 'Format', items: 'bold italic forecolor | formats | removeformat'},
            table: {title: 'Table', items: 'inserttable tableprops deletetable | cell row column'},
            tools: {title: 'Tools', items: 'code'}
        },
        selector: "textarea.tinymce-role",
            ...
            switch(tmce){
                case 'global':
                    $.each(setup,function(key,value) {
                        if( key === 'content_css' ){
                            var arr = new Array(TinyMCE_Global[key]);
                            arr.push(urlBase + '/' + value);
                            value = arr;
                        }
                        TinyMCE_Global[key] = value;
                    });
                    break;
                case 'title':
                    $.each(setup,function(key,value) {
                        if( key === 'content_css' ){
                            var arr = new Array(TinyMCE_Title[key]);
                            arr.push(urlBase + '/' + value);
                            value = arr;
                        }
                        TinyMCE_Title[key] = value;
                    });
                    break;
            }
        });
